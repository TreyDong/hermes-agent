...
1. 飞书日历：今天有什么日程？列出时间、标题
2. 飞书任务：查询所有任务，只列出状态为未完成（done=false）的任务，列出任务标题和截止日期。不要显示已完成的任务。
3. Twitter：检查 @treydtw 今天是否发了推文。
   - 执行命令：`twitter user-posts treydtw --max 3 -j`
   - 用 `date +%Y-%m-%d` 获取当前日期，比对 createdAtLocal 字段（北京时间）是否为今天
...
| 任务 | 截止日期 | 状态 |
|------|----------|------|
| 每日推文 | 2026-04-02 | ⚠️ 已过期 |
| html-slides 风格输出稳定性优化 | 2026-04-02 | ⚠️ 已过期 |
| 围绕日常发布与 Auto Research 建完整内容创作 pipeline | 2026-04-02 | ⚠️ 已过期 |
| 写并发布自己的 skill | 2026-04-02 | ⚠️ 已过期 |
| 每周两次整理知识库，并沉淀创作过程中的知识内容 | 2026-04-02 | ⚠️ 已过期 |
| 研究 Auto Research 与 Harness Engineer 工程 | 2026-04-02 | ⚠️ 已过期 |
| 打造自己的输入输出系统（含固定思考时间） | 2026-04-02 | ⚠️ 已过期 |
| **李霞路联系** | **2026-04-03** | 🔴 **今日到期** |
| 小红书/抖音短视频更新 | 2026-04-05 | 即将到期 |
| 清明后上课 | 2026-04-05 | 即将到期 |
...
最新一条：2026-04-02 22:14（北京）
> "Hermes Agent 的这个自动创建 Skills 的机制真的太强了…"

---

### 📌 今日待办
1. 🔴 **李霞路联系**（今日截止）
2. ⚠️ 处理过期任务（多条 4-2 到期事项需清理或延期）
3. 📝 今日推文待发布
