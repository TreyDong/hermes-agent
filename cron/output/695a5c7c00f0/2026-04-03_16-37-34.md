...
3. Twitter：检查 @treydtw 是否在今天发了推文。使用命令：`twitter user-posts treydtw --max 1`，检查返回的推文日期是否在今天（北京时间）。如果今天已发，列出最新一条的内容概要；如果未发，输出"⚠️ 今日未发推"
4. 汇总：简洁列出今日待办（从飞书任务中找逾期+今日到期的）

飞书日历用 lark-calendar skill，飞书任务用 lark-task skill，Twitter 用 twitter-cli（命令名 twitter）。
...
| 截止日期 | 任务 |
|----------|------|
| 04-02 | html-slides 风格输出稳定性优化 |
| 04-02 | 每日推文 |
| 04-02 | 围绕日常发布与 Auto Research 建完整内容创作 pipeline |
| 04-02 | 写并发布自己的 skill |
| 04-02 | 每周两次整理知识库，并沉淀创作过程中的知识内容 |
| 04-02 | 研究 Auto Research 与 Harness Engineer 工程 |
| 04-02 | 打造自己的输入输出系统（含固定思考时间） |

---

**🐦 Twitter @treydtw**
- ✅ 今日已发（04-02 22:14 北京时间）
- 内容：关于 Hermes Agent 自动创建 Skills 机制的推文，附截图，收获 1 赞
